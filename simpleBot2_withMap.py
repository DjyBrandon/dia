...
class Brain():
    ...
    def __init__(self, botp):
        ...
        self.map = np.zeros((10, 10), dtype=int)  # 10x10 地图
        self.target = None  # 新增目标坐标

    def updateMap(self):
        xMap = int(math.floor(self.bot.x / 100))
        yMap = int(math.floor(self.bot.y / 100))
        xMap = max(0, min(xMap, 9))
        yMap = max(0, min(yMap, 9))
        self.map[xMap][yMap] = 1

    def find_nearest_unexplored(self, x, y, other_maps):
        min_dist = float('inf')
        target = None
        for i in range(10):
            for j in range(10):
    ...
    def thinkAndAct(self, lightL, lightR, chargerL, chargerR, x, y, sl, sr, battery, agents, passiveObjects):
        ...
        if battery < 600:
            ...
            if abs(chargerR - chargerL) < chargerL * 0.1:  # approximately the same
                speedLeft = 5.0
                speedRight = 5.0
        else:  # explore unexplored area (only when enough battery start)
            other_maps = self.bot.getOtherBotsMaps(agents)  # 获取其他机器人的地图数据
            if not self.target or self.map[int(self.target[0] // 100)][int(self.target[1] // 100)] == 1:
                self.target = self.find_nearest_unexplored(x, y, other_maps)
            if self.target:
                newX, newY = self.target
                angle = math.atan2(newY - y, newX - x)
                speedLeft = 0.1 + math.sin(angle - self.bot.theta)
                speedRight = 0.1 - math.sin(angle - self.bot.theta)
...
class Bot():
    ...
    def drawMap(self, canvas):
        canvas.delete(f"map_{self.name}")  # 删除当前机器人专属的旧地图
    ...
    def getOtherBotsMaps(self, agents):
        other_maps = []
        for agent in agents:
            if isinstance(agent, Bot) and agent != self:
                other_maps.append(agent.brain.map)
        return other_maps
    ...
    def thinkAndAct(self, agents, passiveObjects):
        ...
        self.sl, self.sr, xx, yy = self.brain.thinkAndAct \
            (lightL, lightR, chargerL, chargerR, self.x, self.y, self.sl, self.sr, self.battery, agents, passiveObjects)
        if xx != None:
            self.x = xx
        if yy != None:
            self.y = yy
