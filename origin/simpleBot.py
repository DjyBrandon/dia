...
class Brain():
    def __init__(self, bot_position):
        self.bot = bot_position
...
class Bot():
    def __init__(self, name_position):
        self.name = name_position
        self.x = random.randint(100, 900)
        self.y = random.randint(100, 900)
        self.theta = random.uniform(0.0, 2.0 * math.pi)  # [0, 2π]
        self.ll = 60  # 车轴宽度
        self.sl = 0.0
        self.sr = 0.0

    def setBrain(self, brain_position):
        self.brain = brain_position
    ...
    def thinkAndAct(self):  # 根据脑子决定下一步去哪里
...
def createObjects(canvas, noOfBots):
    agents = []
    passiveObjects = []
...
def main():
    ...
    agents, passiveObjects = createObjects(canvas, noOfBots=2)
